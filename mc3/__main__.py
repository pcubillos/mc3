...
def main():
    ...
    if args.cfile is not None and not os.path.isfile(args.cfile):
        print(f"Configuration file: '{args.cfile}' not found.")
        sys.exit(0)
    if args.cfile:
...
def parse():
    ...
    parser.add_argument(
        "-c", "--cfile", help="Configuration file.", metavar="FILE")
    parser.add_argument(
    ...
    group.add_argument(
        "--nsamples", dest="nsamples", action="store", type=eval, default=None,
        help="Number of MCMC samples.")
    group.add_argument(
        "--nchains", dest="nchains", action="store", type=int, default=7,
        help="Number of chains [default: %(default)s]")
    group.add_argument(
        "--ncpu", dest="ncpu", action="store", type=int, default=None,
        help="Number of CPUs for the chains [default: nchains+1]")
    group.add_argument(
        "--sampler", dest="sampler", action="store", type=str, default=None,
        help="Sampler algorithm, select from: ['mrw', 'demc', 'snooker'].")
    group.add_argument(
        "--wlike", dest="wlike", action="store", type=eval, default=False,
        help="Calculate the likelihood in a wavelet base "
             "[default: %(default)s]")
    group.add_argument(
        "--leastsq", dest="leastsq", action="store", type=eval, default=None,
    ...
    group.add_argument(
        "--burnin", dest="burnin", action="store", type=eval, default=0,
        help="Number of burn-in iterations (per chain) [default: %(default)s]")
    group.add_argument(
        "--thinning", dest="thinning", action="store", type=int,  default=1,
        help="Chains thinning factor (use every thinning-th iteration) for GR "
             "test and plots [default: %(default)s]")
    group.add_argument(
        "--fgamma", dest="fgamma", action="store", type=float, default=1.0,
        help="Scaling factor for DEMC's gamma [default: %(default)s]")
    group.add_argument(
        "--fepsilon", dest="fepsilon", action="store", type=float, default=0.0,
        help="Scaling factor for DEMC's support distribution "
             "[default: %(default)s]")
    group.add_argument(
        "--hsize", dest="hsize", action="store", type=int, default=10,
        help="Number of initial samples per chain [default: %(default)s]")
    group.add_argument(
        "--kickoff", dest="kickoff", action="store", type=str, default="normal",
    ...
    group.add_argument(
        "--func", dest="func", action="store", type=mu.parray, default=None,
        help="List of strings with the function name, module "
             "name, and path-to-module [required]")
    group.add_argument(
        "--params", dest="params", action="store", type=mu.parray, default=None,
        help="Filename or list of initial-guess model-fitting "
             "parameter [required]")
    group.add_argument(
        "--pmin", dest="pmin", action="store", type=mu.parray, default=None,
        help="Filename or list of parameter lower boundaries "
             "[default: -inf for each parameter]")
    group.add_argument(
        "--pmax", dest="pmax", action="store", type=mu.parray, default=None,
        help="Filename or list of parameter upper boundaries "
             "[default: +inf for each parameter]")
    group.add_argument(
        "--pstep", dest="pstep", action="store", type=mu.parray, default=None,
    ...
    group.add_argument(
        "--indparams", dest="indparams", action="store",
    ...
    group.add_argument(
        "--data", dest="data", action="store", type=mu.parray, default=None,
        help="Filename or array of the data being fitted [required]")
    group.add_argument(
        "--uncert", dest="uncert", action="store", type=mu.parray, default=None,
        help="Filemane or array with the data uncertainties [required]")
    group.add_argument(
        "--prior", dest="prior", action="store", type=mu.parray, default=None,
        help="Filename or array with parameter prior estimates "
             "[default: %(default)s]")
    group.add_argument(
        "--priorlow", dest="priorlow", action="store", type=mu.parray,
        default=None,
        help="Filename or array with prior lower uncertainties "
             "[default: %(default)s]")
    group.add_argument(
        "--priorup", dest="priorup", action="store", type=mu.parray,
        default=None,
